...
pub async fn get_from_farm(
    ...
    let contract = Contract::from_json(
        web3.eth(),
        farm_address,
        include_bytes!("./abi/farm.json"),
    ).expect("error contract creating");
    let res: (U256,U256,U256,U256,U256) = contract
        .query("getPoolData", farm_index, None, Options::default(), None).await.unwrap();
    // ap tap tc vl 
    ...
    pub async fn update_farms(&self) -> () {
        loop {
            ...
            for farm in farms {
                ...
                let (alloc_point,total_alloc_point,
                    total_claimed,value_locked,mint_per_block
                ) = get_from_farm(
                    &web3, 
