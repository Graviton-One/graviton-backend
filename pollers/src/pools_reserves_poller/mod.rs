...
use crate::schema::{chains, dexes, pools};
use crate::DbPool;
...
impl Pools {
    ...
    fn set_pool_data(pool_id: i64, query: &PoolsUpdate, conn: DbPool) -> () {
...
async fn retrieve_token<T: web3::Transport>(
    ...
    contract
        .query(property, (), None, Options::default(), None)
        .await
}
...
pub async fn get_decimals(address: &Address, web3: Web3Instance) -> i64 {
    let contract = Contract::from_json(web3.eth(), *address, include_bytes!("./abi/erc20.json"))
...
pub async fn get_pool_reserves(
    ...
    let (token_a_reserves, token_b_reserves, _): (U256, U256, U256) = contract
        ...
        .await?;
    use std::str::FromStr;
    Ok((token_a_reserves, token_b_reserves))
}
...
pub async fn get_total_supply(web3: &Web3Instance, pool: Address) -> BigDecimal {
    ...
    let res: U256 = contract
        ...
        .await
        .unwrap();
    BigDecimal::from_str(&res.to_string()).unwrap()
}
    ...
    pub async fn get_pool_data(&self) -> () {
        loop {
            ...
            for pool in pools {
                ...
                let reserves = get_pool_reserves(&pool.pool_address, web3.clone())
                    .await
                    .expect("Error getting pool reserves");
                let price_a: f64 =
                    get_token_price(&pool.coingeco_id, &hex_to_string(&pool.token_a_address)).await;
                let price_b: f64 =
                    get_token_price(&pool.coingeco_id, &hex_to_string(&pool.token_b_address)).await;
                let dec_a =
                    get_decimals(&pool.token_a_address.parse().unwrap(), web3.clone()).await;
                let dec_b =
                    get_decimals(&pool.token_b_address.parse().unwrap(), web3.clone()).await;
                ...
                let total_supply =
                    get_total_supply(&web3, pool.pool_address.parse().unwrap()).await;
